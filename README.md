...
* In the [Makefile](Makefile):
    ...
    * The target `ci_main_pipeline` runs when the provider has a new commit. It performs the following tasks:
        ...
        * If we are on master:
            * Check if we are safe to deploy to prod using `can-i-deploy` (ie. do we have a succesfully verified pact with the version of the consumer that is currently in master)
            * Deploy (just pretend!)
            * Tag the deployed version as `prod` in Pactflow so Pactflow knows which version of the provider is in production when the consumer runs `can-i-deploy`.
...
* In [product/product.pact.test.js](product/product.pact.test.js):
    ...
    * Pact-JS has a very flexible verification task configuration that allows us to use the same code for both the main pipeline and the webhook-triggered verifications, with dynamically set options. Depending on your pact implementation, you may need to define separate tasks for each of these concerns.
