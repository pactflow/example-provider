...
* In [.github/workflows/verify_changed_pact.yml](.github/workflows/verify_changed_pact.yml)
...
* In [src/product/product.pact.test.js](src/product/product.pact.test.js):
  ...
  * The `providerVersionBranch` property is set, so that verification results are published with the branch property which is important for the [contract requiring verification published](https://docs.pact.io/pact_broker/webhooks#the-contract-requiring-verification-published-event) event
